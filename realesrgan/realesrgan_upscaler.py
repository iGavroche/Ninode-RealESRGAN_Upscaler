...
def upscaler(input_img, outscale, gpu_id, tile, fp_fmt):
    ...
    # Set the netscale to the depending one of the model.
    netscale = 4
    ...
    try:
        ...
        #            "CUDA out of memory. Try to set tile size to a smaller number.", duration=5, print_exception=False)
    except UnboundLocalError as error:
        torch.cuda.empty_cache()
        #raise Error("An serious error occurred 💥. " +
        #            "Image size problem occurs. Tile size problem occurs." , duration=5, print_exception=False)
...
class RealEsrganUpscaler:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "gpu_id": (GPU_LIST, {}),
                "tile_number": ("INT", {"default": 0, "min": 0, "max": 8192, "step": 256}),
                "fp_format": (["fp16", "fp32"], {}),
            }
        }
    ...
    def realesrgan_upscaler(self, image, gpu_id, scale_factor, tile_number, fp_format):
        '''Detect ellipse.'''
